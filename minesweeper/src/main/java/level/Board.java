...
public class Board {
    ...
    private void placeMines(int numOfMines, boolean[][] isMine) {
        ...
        while (j < (numOfMines + 1)) {
            if (!this.minesMatrix[(boardHeight / k + 1)][boardWidth % k]) //megn�zz�k nem-e foglalt helyre akarunk tenni
            {
                this.minesMatrix[(boardHeight / k + 1)][boardWidth % k] = true; //az adott helyet akna lesz
